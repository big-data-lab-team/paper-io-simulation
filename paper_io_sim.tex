		...
		\subsection{Principle of the simulator}
			...
			\begin{algorithm}\caption{File read simulation}\label{alg:read}
				...
				\begin{algorithmic}[1]
					...
					\State mm.periodic\_flush() \Comment{Periodically flush expired dirty data}
					\If {mm.cached(fn) $>$ 0}  \Comment{Read file cached data}
    					\State mm.cache\_read(fn)  \Comment{Update cached data}
					\EndIf
					\If {mm.cached(fn) $<$ fs} \Comment{Read uncached data from disk}
						\State sm.read(fs - mm.cached(fn)) \Comment{Simulated disk read}
