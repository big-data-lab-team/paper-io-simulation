...
\tristan{anything else interesting to mention about the implementation?}
        ...
        Our experiments compared real executions with the original WRENCH,
        our WRENCH-cache extension, and our prototype Python simulator. We
        evaluated our page cache simulation in single-threaded and
        multi-threaded applications accessing data on local and network
        file systems.

            We used a pipeline of three sequential tasks where each task reads the output of the previous task,
            increments every byte of the input to emulate real
            processing, and writes the resulting data to disk. The first file was provided as input. We also used
            a real application pipeline \textcolor{red}{[Maybe briefly
            describe this application's purpose (e.g. a standard fMRI
            preprocessing piepeline)]}, to evaluate the applicability of
