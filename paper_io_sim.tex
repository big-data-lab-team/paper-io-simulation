...
\maketitle
    ...
    then evicts cache if needed and adds the written data to the cache.
        ...
        applications, accessing data on local and remote file systems. We
        ...
        Experiments were conducted on a dedicated cluster at
        ...
        Nodes ran CentOS~8.1 with NFS version 4. We used the \texttt{atop}
        ...
        numbered by ascending creation times: File 1 was the file read by Task 1, and file \texttt{i+1} was the file
        written by task \texttt{i}. The anonymous memory used by the application
        ...
        first one (\textit{Exp 1}), we executed \emph{a single instance} of
        ...
        the same local disk.
        In the second experiment (\textit{Exp 2}), we executed
        ...
        The workflow is implemented as a Python script that calls Java
        ...
        \begin{table}
            ...
            \toprule
                \multicolumn{1}{c}{Workflow step}& Input size       & Output size      & CPU time\\
                                       & (MB)             & (MB)             & (s)\\
            \midrule
               Skull stripping         &  111             & 30               & 137 \\
               Tissue classification   &  22              & 175              & 624 \\
               Region extraction       &  175             & 121              & 81 \\
               Cortical reconstruction &  47              & 250              & 270\\
            \bottomrule
            \end{tabular} 
        ...
        We used the real application in the fourth and final experiment
        (\textit{Exp 4}). The workflow was run on a single node of the
        cluster, using a single local disk. We processed data from
        ...
        workflow parameters in Table~\ref{table:nighres_stats}.
    ...
    \begin{figure*}
        ...
        As for the synthetic application, simulation errors are again
        ...
        from 212~\% in WRENCH to 68~\% in WRENCH-cache. The error
        reduction is mostly due to more accurate read times, since write
        times remain generally identical between both implementations, presumably due to 
        the dominance of file compression time in write time.
        Similar to
        the synthetic application, the first read happens entirely from disk and is therefore 
