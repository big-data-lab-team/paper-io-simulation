...
%  Dzung: it's in GB
    ...
    instance, it is now common for HPC clusters to run application jobs in
    Linux control groups (cgroups), where resource consumption, including
    memory and therefore page cache usage, is limited. Using our simulator,
    it would be possible to model the interaction between memory allocation
    and I/O performance, with the goal to improve scheduling algorithms.
    ...
    systems~\cite{islam2015triple}, active storage, or specific
    hardware architectures~\cite{hayot2020performance}. 

    However, I/Os remain much more complex than is currently available in
    simulation models, which could drastically limit their accuracy. A more
    accurate description of the use of anonymous memory by applications
    would be an important extension directly related to our page
    cache model. With the page cache indeed, I/O times are indeed strongly
    dependent on the amount of available memory, which is itself impacted
    by the use of anonymous memory by applications. File access patterns, 
    including random I/O, might also be worth describing, as they directly impact the content of the page cache.
