...
\maketitle
	...
	\begin{abstract}
		...
		The data explosion era seen in recent years results in growing needs in 
		data storing, data analysis. 
		Large computing systems including grids, clusters, clouds, peer-to-peer, 
		volunteer computers have been increasingly used to solve the problem. 
		Despite of their powerful computing capability and technological improvements, 
		there are still bottlenecks in these systems. 
		When it comes to I/O, a bottleneck can be seen in the disk I/O due to hardware 
		limitations. Data caching using page cache is one of the solutions applied 
		to minimize disk accesses and improve the I/O performance. 
		In computing system and application optimization studies, one of the 
		effective approaches is using simulation tools to conduct experiments instead 
		of running applications on real infrastructures. 
		Although simulators are built with individual simulation model for each computing 
		resource, the majority does not take page cache into consideration, 
		which can lead to low simulation accuracy in many cases. \tristan{The
		 ideas are fine, but I don't find them well ordered. I think your intro 
		could adopt this logic (1) Computing systems are key to the processing of Big Data, (2) page 
		cache is critical to I/O performance, (3) page cache is not simulated so how 
		can we expect Big Data applications on computing systems to be well simulated? 
		This problem definition should take 2-3 sentences, the bulk of the abstract should be on the paper, not the context.
		}
		
		In this paper, we propose a simulation model page cache to simulate I/O 
		with key features of page cache. \tristan{The model consists of ...}
		The model is build with the consideration to allow it to be integrated to 
		existing simulation tools. 
		We also implement the model with SimGrid using WRENCH \tristan{people
		 may not know what these things are, characterize them. For instance:
		  "using WRENCH, a SimGrid-based workflow simulator."}. 
		Then we create some simulators for different use cases \tristan{be more specific: characterize the use cases.}. 
		Finally, we analyze the results, compare them to results of the existing 
		tool as well as real execution \tristan{This is too general, 
		you should explain specifically what you found: how is your simulator better than plain simgrid?}.
		\tristan{A conclusion sentence is missing on the impact of your work: how 
		could it be used? what doors does it open? why is it important work?} 
		
	\end{abstract}		 

	\section{Introduction}
		
		In recent years, as a result of the growth of data generated from a variety 
		of data sources, we have seen increasingly growing demands in data storage, 
		processing and analysis. 
		Storing and processing such a massive amount of data require large 
		computing infrastructures. 
		Data processing is a complex process whose the performance is affected 
		by many factors and hindered by bottlenecks in the systems. \tristan{The previous
		sentences are true but a 
		bit general, everybody knows that, it's a boring way to open a paper. 
		Your first sentences could be more specific to the simulation of big data applications.}
		One of the bottlenecks that should be taken into consideration is disk access 
		time due to hardware limitations. 
		Beside hardware technical improvements, architectural solutions have been 
		proposed and implemented in computing systems in order to tackle the 
		disk I/O problem. 
		Caching, a successful technique applied into memory to maximize the 
		I/O performance by keeping data in memory for future accesses,
		has been proving the effectiveness when employed in computer systems. 
		
		\english{Studying in optimizing not only the utilization of page cache but also other 
		computing resources is a need that has never been overlooked.}
		The research on computing infrastructures to maximize performance, 
		shorten processing time, and minimize costs gets involved with experiments. 
		However, real platforms are complex and not built for the experimental 
		purposes due to limited and shared resources, operational costs, unstable 
		environments and the result reproducibility difficulties. 
		An alternative approach is to used simulation tools developed for research 
		and experimental purposes.
		The obvious advantages of using simulation are the freedom of choosing 
		simulated infrastructures, reproducible results and low costs \tristan{The message of the previous 5 sentences could be summarized in 1 or 2.}.
		...
		However \tristan{why "however" and why is this trade-off important?}, some reviews over many simulators report that there has been 
		a trade-off between accuracy and scalability due to the level of details in 
		simulation models \cite{casanova2014simgrid} \cite{byrne2017review}.
		Moreover, different simulators are developed by and for a specific community \tristan{such as? not sure it's important for our purpose},
		which hinders them from being used in cross-domain.
		SimGrid is a simulation framework developed with the objectives of  
		balancing between accuracy and scalability, as well as versatility. 
	...
	\label{relatedwork}	
		...
		\subsection{Simulators}
			...
			\english{There has been the research and development of many simulation tools 
			and frameworks for cloud computing with the goal to simulate cloud, grid, 
			HPC platforms, etc.} 
			\english{A number of the most famous} existing simulators have been analyzed 
			and compared in some reviews (e.g., \cite{byrne2017review}, \cite{casanova2014simgrid}). 
			These simulators provide simulation models of CPU, network, storage 
			with different approaches and levels of details \tristan{what do the reviews conclude?}.
			Some simulators are at packet-level \tristan{for the network, not a general statement}, which allows to achieve 
			high simulation accuracy with fine-grain details such as storage simulation 
			in \textit{DiskSim} or network simulation in \textit{CloudSim} 
			and \textit{iCanCloud} \cite{casanova2014simgrid}. 
			However, the issues with the packet-level simulation are scalability due to 
			long simulation time when the simulated platforms and applications grow 
			to hundreds and thousands nodes. 
			...
			However, I/O with page cache model has not been provided in SimGrid \tristan{has it been in other simulators?}. 
			It only simulates storage with capacity and bandwidths.
			Lebre \textit{et al.} \tristan{it looks like you're targeting this author, just use the reference as for the other works} adds a storage simulation extension to SimGrid 
			to simulate file system,  but the page cache is not taken into account 
			\cite{lebre2015}. 
			SIMCAN is the \tristan{the or a?} simulation framework that models I/O with page cache 
			\english{with data on disk can be kept in page cache} \cite{nunez2012simcan} \tristan{This paragraph started with Simgrid, so it gives the impression that 
			all these simulators are in simgrid but I don't think they are. Revise.}.
			Memory is also modeled in iCanCloud simulator with a component in charge 
			...
			Xu \textit{et al.} applies four cache replacement policies to simulate in-memory 
			caching, but for the study in energy consumption of multi-tier heterogenous 
			network \cite{xu2018saving}.
	...
	\label{method}	
		...
		\subsection{Principle of the simulator}
			...
			\begin{figure}
				   ...
				   \caption{Overview of the simulated write-back page cache \tristan{as discussed before, it would be useful to represent 
				   which functions are exposed by each object. To understand the interface of each object. Also, add a line of explanation in the caption so 
				   that we understand the main concept better by just looking at the figure.}}\label{fig:interaction}
			\end{figure}

			\subsubsection{Memory simulation model}
			\tristan{in this section there needs to be a clearer mapping between functions shown in Fig 1 and the paragraphs describing them.
			Maybe add paragraph headers that match function names in Fig 1.}
			
			We leverage existing storage simulation models to simulate the memory,
			which is characterized with storage capacity, \english{distinguished} read/write 
			bandwidths and device latency. 
			\english{All of these features are dealt within models in simulation 
			...
			Memory Manager simulates two parallel threads \tristan{refer to figure 1}. The first thread is the 
			main thread where activities of applications are simulated \tristan{unclear, what are "activities of applications"?}, and responsible 
			...
			to disks \tristan{how about periodical eviction?}. 
			...
			\english{Now,} page cache LRU lists can be modeled with two lists of data blocks: 
			...
			numerous than file pages, \english{while achieving the accuracy in I/O time
			simulation that can be seen in the later section}.
			
			\english{Given an arbitrary file, at a specific time, it can be partially cached, 
            ...
            located in the inactive list that are subsequently accessed, 
            will be moved to the top of the active list. \tristan{The previous 2 sentences could be made clearer} Cached blocks 
            written to cache are marked as dirty blocks until they are flushed to disk.
            ...
            after all uncached data is read \tristan{why did you use this order?} (left to right in Figure~\ref{fig:read_order}).
			...
			a single clean block and then also put in the active list \tristan{The
			construction of the sentence insists on the difference between dirty and clean blocks, it would be clearer to first explain what is common, then mention what is different.}. 
			...
			In case a block is not entirely read \tristan{why would this happen?}, it is split into two smaller blocks, 
			...
			to simulate I/O time \tristan{some of this could be moved to the top of II.A. It would be important to understand early on 
			what are the relations between the memory manager and the io controller.}.
			...
			When a simulated application reads a file chunk, it sends a 
			chunk read request to IO Controller \tristan{say that Alg 2 implements the IOcontroller read method, here and in the title and in Fig 1.}. 
			...
			flush dirty data to disk and simulate flushing time with memory model (line 9). \tristan{do you assume that nothing is stored in anonymous memory when file read is called? 
			it's ok but you should explain that this is due to the lack of application memory management in simgrid.}
			...
			\english{update the corresponding data blocks in cache (line 16). }
		...
		\subsection{Implementation}
			...
			Since bandwidth sharing is not simulated, this prototype does not support 
			...
			the core of the model separately from existing WRENCH components \tristan{why?}, 
