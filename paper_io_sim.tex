...
\author{\IEEEauthorblockN{Hoang-Dung Do$^1$, Val\'erie Hayot-Sasson$^1$, Rafael Ferreira da Silva$^3$, Henri Casanova$^2$, Tristan Glatard$^1$
  ...
  \IEEEauthorblockA{
    $^1$Department of Computer Science and Software Engineering, Concordia University, Montreal, Canada \\
     $^2$Department of Information and Computer Sciences, University of Hawai‘i at Manoa, U.S.A \\
...
\maketitle
	...
	\begin{abstract}
		...
		which can lead to low simulation accuracy in many cases. 
		
		In this paper, we propose a simulation model page cache to simulate I/O 
		with key features of page cache. 
		The model is build with the consideration to allow it to be integrated to 
		existing simulation tools. 
		We also implement the model with SimGrid using WRENCH. 
		Then we create some simulators for different use cases. 
		Finally, we analyze the results, compare them to results of the existing 
		tool as well as real execution. 
	...
	\section{Introduction}
		...
		by many factors and hindered by bottlenecks in the systems.
		...
		Studying in optimizing not only the utilization of page cache but also other 
		...
		However, some reviews over many simulators report that there has been 
		...
		Moreover, different simulators are developed by and for a specific community,
		...
		but they focus on very specific domains \cite{xu2018saving}.
		
		In this paper, we propose a simulation model of I/O with page cache.
	...
	\label{relatedwork}	
		...
		\subsection{Page cache}							
			...
			In Linux, page cache is a part of memory, which includes pages referring to 
			physical pages on disk \cite{linuxdev3rd2010}. 
			...
			page cache to disk is. In this case, data is written to both page cache and disk. 
			In the third strategy, which is \textit{write-back}, data are written directly 
			...
			direct I/O since it delays disk writes and perform in batch later 
		...
		\subsection{Simulators}
			...
			There has been the research and development of many simulation tools 
			and frameworks for cloud computing with the goal to simulate cloud, grid, 
			HPC platforms, etc. 
			A number of the most famous existing simulators have been analyzed 
			...
			with different approaches and levels of details.
			Some simulators are at packet-level, which allows to achieve 
			...
			However, I/O with page cache model has not been provided in SimGrid. 
			It only simulates storage with capacity and bandwidths.
			Lebre \textit{et al.} adds a storage simulation extension to SimGrid 
			to simulate file system,  but the page cache is not taken into account 
			\cite{lebre2015}. 
			SIMCAN is the simulation framework that models I/O with page cache 
			with data on disk can be kept in page cache \cite{nunez2012simcan}.
			...
			framework to be extended with new models. 
			...
			The first is the process control, synchronization of resource consumption by 
			concurrent processes handled by the simulation core in SimGrid. 
			The second is the existing SimGrid storage model with capacity and 
			bandwidth properties, which are similar to the characteristics of memory.
			The third is dropping the burden of implementation with the high level 
