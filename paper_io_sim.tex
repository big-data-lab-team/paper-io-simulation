...
\maketitle
    ...
    \begin{figure}
           ...
           \caption{Overview of the simulated writeback page cache \tristan{does it simulate only writeback? See comment in NFS section.}.
    ...
    \begin{figure}
           ...
           is read first, followed by data from the inactive list, and finally data from the active list. \tristan{Colors 
           shouldn't match that in Fig 1 as they dont represent the same thing.}}
    ...
    \texttt{flush} and \texttt{evict} simply return and do not do anything. Then, if the block requires
    uncached data, a disk read is simulated with the disk model (line 12)
    \tristan{this is not consistent with the new Figure 1 where disk reads
    are done by the mm}, and the data read is added to cache (line
    13).
    ...
    \label{alg:write}
        ...
        \begin{algorithmic}[1]
            ...
            \If {mem\_amt $<$ cs}  \Comment{Write with disk bandwidth}
                ...
                \State sm.write(cs - mem\_amt) \tristan{According to Fig 1 the IO Controller is not supposed to interact directly with sm}
            \EndIf
    ...
    data from cache if necessary (line 8) \tristan{the difference between available and free memory should be explained the first time it is referred}.
    ...
    and the chunk write algorithm finishes \tristan{see comment in algorithm}.
        ...
        \subsection{Implementation}
            ...
            We exposed page cache simulation through a user API, activated with a command-line argument.
...
\tristan{anything else interesting to mention about the implementation?}
        ...
        bandwidth values in our experiments.
            ...
            previous one, albeit reading and writing on a 400-GB remote NFS disk
            ...
            cache \tristan{it it part of the IO Controller? it would deserve
    ...
    \label{results}
        ...
        for all I/O operations except the first read as it only involves
        uncached data. Errors are reduced from an average of \texttt{xxx\%}
        in WRENCH to \texttt{yyy \%} in the Python prototype and
        \texttt{zzz \%} in WRENCH-cache \tristan{Dzung, update average percentages}.
