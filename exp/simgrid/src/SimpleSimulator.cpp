...
int main(int argc, char **argv) {
    ...
    if (argc != 4) {
        std::cerr << "Usage: " << argv[0] << " <xml platform file> <workflow file> <number of pipelines>" << std::endl;
        exit(1);
    }
    ...
    int no_pipe = std::atoi(argv[3]);

    // Reading and parsing the workflow description file to create a wrench::Workflow object
    std::cerr << "Loading workflow..." << std::endl;
//    wrench::Workflow *workflow = workflow_exp1(workflow_file);
    wrench::Workflow *workflow = workflow_exp2(no_pipe, 3, 1, 5000000000, 3000000000, 3000000000);
...
//  std::string file_registry_service_host = hostname_list[(hostname_list.size() > 2) ? 1 : 0];
    ...
    simulation.getOutput().dumpUnifiedJSON(workflow, "export/unified_" + to_string(no_pipe) + ".json");

//    export_output_exp2(simulation.getOutput(), workflow->getNumberOfTasks(),
//            "timestamp_sim_exp2_100gb.csv");
